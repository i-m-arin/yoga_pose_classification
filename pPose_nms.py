...
def write_json(all_results, outputpath, for_eval=False):
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            result = {}
            if for_eval:
                result['image_id'] = int(im_name.split('/')[-2:].split('.')[0].split('_')[-1])
            else:
                some = im_name.split('/')[-2:]
                result['image_id'] = "/".join(some)
    ...
    if form == 'cmu': # the form of CMU-Pose
        with open(os.path.join(outputpath,'keypoints.json'), 'w') as json_file:
